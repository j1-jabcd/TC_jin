...
Car_KinoSearch::Car_KinoSearch(ros::NodeHandle& nh){
    ...
    nh.param("car_search/car_l",car_l,0.8);
    nh.param("car_search/car_w",car_w,0.3);
    nh.param("car_search/car_h",car_h,0.2);
    ...
    O----O
    */
    kinosearchPub = nh.advertise<visualization_msgs::MarkerArray>("/visualization/car_kinosearch", 1);//记消息用于将可视化“标记”（例如框、球体、箭头、线等）发送到可视化环境（例如rviz）
    point_cloud_sub_ = nh.subscribe("global_map", 10, &Car_KinoSearch::GlobalMapBuild, this);//订阅全局地图，受到地图后进入个GlobalMapBuild
    ...
    path_node_pool_.resize(allocate_num_);//对路径池进行初始化
    for (int i = 0; i < allocate_num_; i++) {
      path_node_pool_[i] = new PathNode;
    }
    shotptr =std::make_shared<ompl::base::ReedsSheppStateSpace>(MinTurnRadius);//智能指针/最小转弯半径
       // auto (std::make_shared<ob::SE3StateSpace>());
     //space =std::make_shared<ompl::base::DubinsStateSpace>(MinTurnRadius);//智能指针/最小转弯半径
...
void Car_KinoSearch::GlobalMapBuild(const sensor_msgs::PointCloud2 & pointcloud_map){//global_map回调函数
    if(has_map) return;
    pcl::PointCloud<pcl::PointXYZ> cloud;
    pcl::fromROSMsg(pointcloud_map, cloud);//将ros 的点云数据转换成pcl点云库数据
    ...
    has_map = true;
    ROS_WARN(" jin Sim Detection Node: Finish gridmap built");
}
...
void Car_KinoSearch::stateTransit(Eigen::Vector3d& state0,//向下一个状态转换根据steer
...
int Car_KinoSearch::car_search(Eigen::Vector3d start_state,Eigen::Vector3d end_state){
    ...
    while (!open_set_.empty()) {
        ...
        is_shot_sucess(cur_node->state,end_state);
                        std::cout << "[Kino Astar]: near end.jin"  << std::endl;
        if (!is_shot_succ_){
            return NO_PATH;
                                                std::cout << "fuch end"  << std::endl;
}
        else{
