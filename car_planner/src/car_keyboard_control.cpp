...
void chatterCallback(const geometry_msgs::Twist::ConstPtr& msg1);
...
int main(int argc, char **argv)
{
    ...
    nh_priv.param("car/wheelbase",wheelbase,0.6);
  ...
  //  waypoints_sub= nh_priv.subscribe( "waypoints", 1, rcvWaypointsCallback );
    state_timer = nh_priv.createTimer(ros::Duration(0.01),pub_carstate);
   ...
   ros::Subscriber twis_sub = nh_priv.subscribe<geometry_msgs::Twist>("cmd_vel",1, chatterCallback);
...
void chatterCallback(const geometry_msgs::Twist::ConstPtr& msg1)
{
    double dt = 0.1;
        if(msg1->angular.z!=0){
        double k = 0.5/wheelbase*tan(msg1->angular.z);//k=0?
        current_state[0] = current_state[0]+msg1->linear.x*sin(current_state[2]+k*dt)/k-msg1->linear.x*sin(current_state[2])/k;
        current_state[1] = current_state[1]+msg1->linear.x*cos(current_state[2])/k-msg1->linear.x*cos(current_state[2]+k*dt)/k;
        current_state[2] = current_state[2]+k*dt;
        current_state[2] = normalizeTheta(current_state[2]);
    }
    else{
        current_state[0] = current_state[0]+msg1->linear.x*dt*cos(current_state[2]);
        current_state[1] = current_state[1]+msg1->linear.x*dt*sin(current_state[2]);
        current_state[2] = current_state[2];
        current_state[2] = normalizeTheta(current_state[2]);
    }
        ROS_INFO_STREAM("TARGET=get");
...
void pub_carstate(const ros::TimerEvent& event){
    ...
    car_odom.pose.pose.orientation.w = 10;//q.w();
